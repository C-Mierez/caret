...
import { quickEditCaller } from "./caller";
...
export const quickEditState = StateField.define<boolean>({
	...
	update(value, transaction) {
		...
		if (transaction.selection) {
			...
			if (selection.empty) {
				return false; // Hide quick edit if selection is empty
			}
		}
		return value;
	},
...
function createQuickEditTooltip(state: EditorState): readonly Tooltip[] {
	...
	return [
		{
			...
			create() {
				...
				dom.className =
					"bg-popover text-popover-foreground z-50 p-2 flex flex-col gap-2 text-sm";
